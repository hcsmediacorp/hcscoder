...
fn benchmark_api_key_validation(c: &mut Criterion) {
    c.bench_function("validate_api_key_valid", |b| {
        b.iter(|| {
            hcscoder::hcscoder_openrouter::auth::validate_api_key(black_box(
                "sk-or-a1b2c3d4e5f6g7h8",
            ))
        })
    });

    c.bench_function("validate_api_key_invalid", |b| {
        b.iter(|| hcscoder::hcscoder_openrouter::auth::validate_api_key(black_box("invalid-key")))
    });
}
